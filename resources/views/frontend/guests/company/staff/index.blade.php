...
@section('content')
    ...
    <div class="table-wrapper">
        ...
        <table class="table-address">
            <thead>
            <tr>
                <th width="40px"><input id="allSelect" type="checkbox"/>全选</th>
                {{--<th width="60px">工号</th>--}}
                <th width="40px">姓名</th>
            ...
            @if($type == 'done')
                @foreach($data as $k=>$v)
                    <tr>
                        ...
                        <td>{{explode(' ',$v->start_time)[0]}}</td>
                        {{--<td>2018-06-05</td>--}}
                        <td><button class="btn btn-edite" myId = "{{$v->id}}">修改</button><button class="btn red del" myId = "{{$v->id}}">
                                <a href="/staff/passStaff/{{$v->id}}">删除</a></button></td>
                    </tr>
                @endforeach
            @elseif($type == 'add')
                @foreach($data as $k=>$v)
                    <tr>
                        ...
                        <td>{{$v->date}}</td>
                        {{--<td>2018-06-05</td>--}}
                        <td><button class="btn btn-edite" myId = "{{$v->id}}">修改</button><button class="btn red del" myId = "{{$v->id}}"><a href="/staff/delStaff/{{$v->id}}">删除</a></button></td>
                    </tr>
                @endforeach
            @else
                @foreach($data as $k=>$v)
    ...
    <div class="popup popup-person">
        ...
        <div class="popup-wrapper">
            ...
            <form action="/staff/editStaff" method="post" enctype="multipart/form-data">
                {{ csrf_field() }}
            ...
            <div class="popup-content">
                <ul class="form-wrapper">
                    ...
                    <li>
                        ...
                        <button class="btn-00a2ff btn-primary"><a href="/download/group_excel/团险被保人模板.xlsx">exce模板</a></button>
                        <i class="error"></i>
                    </li>
                    <li>
...
//            console.log(e.target.result)
    ...
    $(function(){
        ...
        $('.btn-save').click(function(){
            ...
            checkCorrect($('#name'),nameReg);
            checkCorrect($('#idCard'),IdCardReg);
