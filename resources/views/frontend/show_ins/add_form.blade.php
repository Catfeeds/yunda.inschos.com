...
@section('content')
    ...
    <form class="form-horizontal" method="post"  id="form" action="{{ url('product/confirmform')}}" enctype="multipart/form-data">
                                    ...
                                    <ul>
                                        @foreach($v['productAttributes'] as $vk => $vv)
                                            <li id="{{'b'.$vv['type']}}">
                                                <span class="name"><i class="f18164">*</i>{{$vv['name']}}</span>
                                                @if($vv['type'] == 1)
                                                    <input type="date" id="{{$vv['ty_key']}}"  style="font-size: 16px;color:#888888 ; " name="insurance_attributes[{{ $v['module_key'] }}][{{ $beibaoren_key }}][{{ $vv['ty_key'] }}]" class="common-input" value="{{date('Y-m-d',time())}}">
                                                @endif
                                                @if($vv['type'] == 3)
                                                    <input type="text" class="common-input w320" placeholder="{{$vv['name']}}" id="{{ $vv['ty_key'] }}" name="insurance_attributes[{{ $v['module_key'] }}][{{ $beibaoren_key }}][{{ $vv['ty_key'] }}]">
                                                    <span class="error"></span>
                                                @endif
                                                @if($vv['type'] == 4)
                                                ...
                                                @if($vv['type'] == 6)
                                                    @foreach($vv['attributeValues'] as $ak => $av)
                                                        <input class="btn btn-default" name="insurance_attributes[{{ $v['module_key'] }}][{{ $beibaoren_key }}][{{ $vv['ty_key'] }}]" value="{{$av['ty_value']}}" />{{$av['value']}}
                                                    @endforeach
                                                @endif
                                                @if(in_array($vv['type'], [0, 9]))
