						...
						<ul>
							...
							@if(count($channel_users)!=0)
								@foreach($channel_users as $user)
									<li class="ui-table-tr">
										<div class="col-md-1">{{$user['channel_user_info']['channel_user_name']}}</div>
										<div class="col-md-1">身份证号</div>
										<div class="col-md-2">{{ $user['channel_user_code']?? $user['channel_user_info']['channel_user_code']}}</div>
										<div class="col-md-1 color-negative"><i class="iconfont icon-shiming"></i>已实名</div>
										<div class="col-md-1">{{ $user['channel_user_info']['channel_user_phone'] }}</div>
										<div class="col-md-1">{{count($user['channel_user_info'])}}</div>
										<div class="col-md-1">{{count($user['channel_user_info'])*2}}</div>
										<div class="col-md-1">{{count($user['channel_user_info'])*2*49/100}}</div>
