...
//        dd($product_list);
        foreach($product_list as $k=>$v){
            $v['rate'] = $this->getMyAgentBrokerage($v->ty_product_id,$ditch_id,$agent_id);
...
//        dd($cust);
        return view('frontend.agents.agent_plan.add_plan',compact('cust','count','product_list','product_count','jurisdiction','option','authentication'));
    }
    /**
     * 获取基础费率
     *
...
//        $url = url('')
    ...
    public function planLists()
    {
        ...
        foreach($lists as $k=>$v){
            ...
            $v->rate = $this->getMyAgentBrokerage($v->ty_product_id,$this->getMyDitch($_COOKIE['agent_id']),$_COOKIE['agent_id']);
...
//        dd($data);
    ...
    public function planDetailOther($id)
    {
        $data = PlanLists::where('id',$id)
            ...
            ->first();
        $data->rate = $this->getMyAgentBrokerage($data->ty_product_id,$this->getMyDitch($_COOKIE['agent_id']),$_COOKIE['agent_id']);
//        dd($data);
...
//        dd($detail);
        ...
        $detail->rate = $this->getMyAgentBrokerage($prodcut_id->ty_product_id,$ditch_id,$agent_id);
//        dd($detail);
        $authentication = $this->isAuthentication($_COOKIE['agent_id']);
...
//        dd($data);
    ...
    public function searchProduct()
    {
        ...
        $product_list = $this->getMyAgentProduct($agent_id);//获取所有的产品列表
        foreach($product_list as $k=>$v){
            $v['rate'] = $this->getMyAgentBrokerage($v->id,0,$agent_id);
            $v['premium'] = $this->getProductPremium($v->ty_product_id);
            foreach(json_decode($v->json,true)['clauses'] as $kk=>$vv){
                if($vv['type'] == 'main')
                {
...
//        dd($data);
        foreach($data as $v){
            ...
            $v->brokerage = $this->getMyAgentBrokerage($v->ty_product_id,1,$_COOKIE['agent_id']);
            $v->premium = $this->getProductPremium($v->ty_product_id);
...
//        dd($count_sales);
        ...
        $data['rate'] = $this->getMyAgentBrokerage($data['ty_product_id'],1,$_COOKIE['agent_id']);
        //获取产品的条款
        $biz_content = [
