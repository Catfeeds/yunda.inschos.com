    ...
    <ul class="form-wrapper">
        <li>
            ...
            @if(!empty($information['phone']))
                <input id="tel" type="text" name="phone" maxlength="11" value="{{$information['phone']}}" />
            @else
                <input id="tel" type="text" name="phone" maxlength="11" placeholder="请输入常用手机号码" />
            @endif
            <a class="blue" href="#"><span><i class="iconfont icon-wancheng"></i>修改</span></a>
            <i class="error"></i>
        </li>
        <li>
        ...
        <li>
            @if(!empty($information['inAddress']))
                <span class="form-name"></span><textarea name="in_address" >{{$information['inAddress']}}</textarea>
            @else
                <span class="form-name"></span><textarea name="in_address" placeholder="建议如实填写地址，方便您收取保单，例如 街道名称，门牌号，楼层和房间号等信息"></textarea>
            @endif
        </li>
        <li>
