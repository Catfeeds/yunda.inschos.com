...
//        }
    ...
    public function doInsureWechatPay(){
        set_time_limit(0);//永不超时
        $channel_contract_info = ChannelContract::where('is_valid','<>','1')//有效签约
...
//                     ]);
    ...
    public function warrantyListAuto(){
        $order_res = ChannelOperate::where('pay_status','200')
        ->select('channel_user_code','proposal_num')
        ->get();
