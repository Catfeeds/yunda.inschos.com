...
class IndexController extends BaseController
{
	...
	public function getParams()
	{
		...
		$biz_content['work_start'] = date('Y-m-d H:i:s', time());
		$biz_content['is_word'] = '1';//1上工，0没上工
		if (!is_array($biz_content) && is_object($biz_content)) {
			$biz_content = $this->object2array($biz_content);
		}
		if ($biz_content['channel_code'] == "YD") {
			$insured_name = isset($biz_content['channel_user_name'])?empty($biz_content['channel_user_name'])?"":$biz_content['channel_user_name']:"";
			$insured_code =isset($biz_content['channel_user_code'])?empty($biz_content['channel_user_code'])?"":$biz_content['channel_user_code']:"";
			$insured_phone = isset($biz_content['channel_user_phone'])?empty($biz_content['channel_user_phone'])?"":$biz_content['channel_user_phone']:"";
			//姓名，身份证信息，手机号判空
			if(!$insured_name||!$insured_code||!$insured_phone){
				return json_encode(['status' => '500', 'content' => '请检查 姓名，证件号，手机号 是否正确！'], JSON_UNESCAPED_UNICODE);
			}
			//判断person表里有没有此人信息，人员基础信息入库
			$repeat_res = Person::where('papers_code',$biz_content['channel_user_code'])->select('phone')->first();
			if(empty($repeat_res)){
				Person::insert([
					'name'=>$insured_name,
					'papers_type'=>'1',
			...
			$is_word = '1';//是否上工
			$work_start = date('Y-m-d H:i:s', time());//上工时间
			//TODO 匹配出没有签约的和签约过期的
			$channel_contract_res = ChannelContract::where('channel_user_code', $insured_code)
				->where('is_valid', '0')//todo 签约协议失败
	...
	//todo 测试出单操作
	public function insureIssueTest($order_id, $union_order_code)
	{
		$warranty_rule = WarrantyRule::where('order_id', $order_id)->first();
		$i = new Issue();
	...
	//todo 测试出单操作
	public function insureIssueTests()
	{
		set_time_limit(0);//永不超时
		$i = new Issue();
	...
	{
		$url = 'http://' . $_SERVER['HTTP_HOST'] . 'channelsapi/' . $name;
		switch ($name) {
			case 'insure_info':
				return $url;
				break;
	...
	public function doInsure()
	{
		...
		//TODO 匹配出没有签约的
		//TODO  匹配出签约过期的
		$channel_contract_res = ChannelContract::where('channel_user_code', $person_code)
			->where('is_valid', '0')
			->select('openid', 'contract_id', 'contract_expired_time', 'is_auto_pay')//openid,签约协议号,签约过期时间
			->first();
		$contract_expired_time = strtotime($channel_contract_res['contract_expired_time']);
		if (empty($channel_contract_res)) {
			$warranty = false;//没有签约
			...
			$warranty = false;//签约已过期
			$insure_auto_pay = true;
		} elseif ($channel_contract_res['is_auto_pay'] == '1') {
			$warranty = true;//已经签约
			$insure_auto_pay = false;//自动投保状态为关闭
		} else {
			$warranty = true;//已经签约
			$insure_auto_pay = true;
		}
	...
	{
		return view('frontend.channels.setup_list');
	}
	...
	public function userSetup()
	{
		...
		$person_code = $access_token_data['person_code'];
		if (!$person_code) {
			return view('frontend.channels.user_setup')
				->with('access_token_data', $access_token_data);
		}
		$user_res = ChannelPrepareInfo::where('channel_user_code', $person_code)->select('channel_user_name', 'channel_user_phone', 'channel_user_code')->first();
		if (empty($user_res)) {
			return view('frontend.channels.user_setup')
				->with('access_token_data', $access_token_data);
		} else {
			return view('frontend.channels.user_setup')
				->with('access_token_data', $access_token_data)
				->with('user_res', $user_res);
		}
	}

	//快递保设置
	public function insureSetup()
	{
		return view('frontend.channels.insure_setup');
	}
	...
	public function toInsure()
	{
		...
		$data['insurance_attributes'] = '';
		$data['union_order_code'] = $union_order_code;
		$data['pay_account'] = $access_token_data['person_code'] ?? $access_token_data['person_phone'];
		$data['clientIp'] = $ip;
		$data = $this->signhelp->tySign($data);
		//发送请求
		$response = Curl::to(env('TY_API_SERVICE_URL') . '/ins_curl/contract_ins')
	...
	public function getWarranty()
	{
		...
		$channel_operate_res = ChannelOperate::where('channel_user_code', $person_code)
			->where('pay_status', '200')
			->select('proposal_num')
			->get();
		$warranty_ids = [];
		foreach ($channel_operate_res as $value) {
			$warranty = WarrantyRule::where('union_order_code', $value['proposal_num'])->select('warranty_id')->first();
			if (!empty($warranty['warranty_id'])) {
				$warranty_ids[] = $warranty['warranty_id'];
			} else {
				$warranty_ids[] = '1';
			}
		}
...
//        print_r($response);die;
	...
	public function contractCallBack()
	{
		...
		$channel_contract_res = ChannelContract::where('channel_user_code', $channel_user_code)->select('openid')->first();
		if (empty($channel_contract_res)) {
		...
		} elseif ($input['openid'] != $channel_contract_res['openid']) {
