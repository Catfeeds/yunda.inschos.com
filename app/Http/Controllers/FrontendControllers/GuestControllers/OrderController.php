    ...
    public function getOrderList($status,$id)
    {
        if(is_integer($status)){//获取特定条件的订单
            ...
            if($status == 'all'){
                ...
                $order_list = Order::with('product','warranty_rule.warranty','warranty_recognizee')->where([['user_id',$id],['status',config('attribute_status.order.payed')],['end_time','<',date('Y-m-d H:i:s')]])->paginate(config('list_num.frontend.order'));;
            }
        }
