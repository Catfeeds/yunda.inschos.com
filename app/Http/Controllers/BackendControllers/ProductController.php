...
class ProductController extends Controller
{
    ...
    public function getProducts(Request $request,$input)
    {
        ...
        $product_ids = Product::where('status','1')->get();
        $ty_product_id = [];
...
//        Cache::forever('ty_product_res_'.$page, $return_data,$expiresAt);
    ...
    public function productStayOn(Request $request)
    {
        $input = $request->all();
        $page = isset($input['page']) ? $input['page'] : 1;
        $products = Product::where('delete_id', '0')->select('ty_product_id')->get();
        $product_id = [];
        foreach ($products as $product) {
            $product_id[] = $product['ty_product_id'];
        }
        $products_up = Product::where('delete_id', '0')->where('status','1')->select('ty_product_id')->get();
        $product_up_id = [];
...
//        dd($product_res['clauses']);
    ...
    {
        $request = $this->request->all();
        $product_res = Product::where('sale_status','1');
        $where = array();
        if(isset($request['keyword'])){
            $keyword = $request['keyword'];
            switch($request['search_type']){
...
//        dd($product_res);
    ...
    public function productRec()
    {
        ...
        $res = Product::where('delete_id', '1')->paginate($limit);
