...
@section('title')@parent代理商系统 @stop
@section('head-more')
    ...
    <div class="row">
        ...
        <div class="col-lg-2 col-md-4">
            <div class="info-wrapper">
                ...
                <div class="info-item">
                    ...
                    <div>保费：<span class="price">{{ceil($today['premium']??"0"/100)}}</span></div>
                    <div>件数：<span class="price color-primary">{{$today['count']??"0"}}</span></div>
                </div>
                <div class="info-item">
                    <div class="name color-primary">昨日</div>
                    <div>保费：<span class="price">{{$yesterday['premium']??"0"/100}}</span></div>
                    <div>件数：<span class="price  color-primary">{{$yesterday['count']??"0"}}</span></div>
                </div>
            </div>
        </div>
                    ...
                    <select class="form-control" style="display: inline-block;" id="premium_date">
                        @if(isset($time)&&!empty($time))
                            @foreach($time as $value)
                                <option value="{{date('Y',strtotime($value['created_at']))}}">{{date('Y',strtotime($value['created_at']))}}年</option>
                            @endforeach
                        @endif
                    </select>
                </div>
            ...
            <div class="chart-wrapper">
                <div class="select-wrapper">
                    ...
                    <select class="form-control" style="display: inline-block;" id="twoTime">
                            @if(isset($twoTime)&&!empty($twoTime))
                                @foreach($twoTime as $value)
                                    <option value="{{date('Y',strtotime($value['created_at']))}}">{{date('Y',strtotime($value['created_at']))}}年</option>
                                @endforeach
                            @endif
                    </select>
        ...
        <div class="col-lg-2 col-md-4">
            <div class="info-wrapper">
                ...
                <div class="info-item">
                    <div class="name">今日</div>
                    <div>佣金：<span class="price">{{ceil($t_brokerage??"0"/100)}}</span></div>
                </div>
                <div class="info-item">
                    <div class="name color-primary">昨日</div>
                    <div>佣金：<span class="price">{{ceil($y_brokerage??"0"/100)}}</span></div>
                </div>
            </div>
        </div>
    </div>
    <div class="row">
        ...
        <div class="col-lg-2 col-md-4">
            <div class="info-wrapper">
                ...
                <div class="info-item">
                    <div class="name">今日</div>
                    <div>客户数：<span class="price">{{$t_customer??"0"}}</span></div>
                </div>
                <div class="info-item">
                    <div class="name color-primary">昨日</div>
                    <div>客户数：<span class="price">{{$y_customer??"0"}}</span></div>
                </div>
            </div>
        </div>
            ...
            <div class="chart-wrapper">
                <div class="select-wrapper">
                    ...
                    <select class="form-control" style="display: inline-block;" id="agentTime">
                        @if(isset($timeAgent)&&!empty($timeAgent))
                            @foreach($timeAgent as $value)
                                <option value="{{date('Y',strtotime($value['created_at']))}}">{{date('Y',strtotime($value['created_at']))}}年</option>
                            @endforeach
                        @endif
                    </select>
        ...
        <div class="col-lg-2 col-md-4">
            <div class="info-wrapper">
                ...
                <div class="info-item">
                    <div class="name"></div>
                    <div>总任务额度</div>
                    <div class="price">{{ceil($task??"0"/100)}}</div>
                </div>
                <div class="info-item">
                    <div class="name color-primary"></div>
                    <div>任务完成额度</div>
                    <div class="price">{{ceil($finish??"0"/100)}}</div>
                </div>
            </div>
        </div>
    </div>
