        ...
        <div class="introduce-parameter-row clearfix" @if(!$v['display']) style="display: none;" @endif>
            <ul class="content-list">
                ...
                @elseif (in_array($v['type'], [1]))
                    <li class="pickerfour">
                        <span class="bold">{{$v['name']}}</span>
                        <input id="date" type="text"  value="{{ $s_arr[$v['ty_key']] ?: '' }}"  data-value='{"protectItemId":"{{$protectItemId}}","key":"{{$v['ty_key']}}","value":"{{$s_arr[$v['ty_key']]}}","sort":{{$v['sort']}}}' type="text" placeholder="请选择">
                        <i class="iconfont icon-gengduo"></i>
                    </li>
                @endif
            </ul>
...
//                    $('.introduce-parameter-row ul li button').on('click', function (event) {
//                        var $this = $(this);
//                        that.oldVal = $this.find('button.label-active').attr('data-value');   //变更选项的旧值
//                        $this.find('button').removeClass('label-active');
//                        $this.find('button:label-active').addClass('label-active');
//                        $("#quote-button").trigger('click');
//                    });
