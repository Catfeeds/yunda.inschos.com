...
class DitchAgentController extends BaseController
{
    ...
    public function channel()
    {
        $id = $_GET['id'] ?? "";
        $name = $_GET['name'] ?? "";
        $a =Ditch::with(['agents','task_detail','order_brokerage']);
        $name && $a->where('name', 'like', '%'.$name.'%');
        $id && $a->where('id',$id); //$id &&的意思是  $id  不为空的话 才会执行后面的 where 语句
        $a->where('status','on');
        $a->where('status','on');
        $a->select(DB::raw('*, concat(path,id) as npath'));
        $a->orderBy('npath', 'asc');
        $channel = $a ->paginate(30);
//        读取代理人
...
//        佣金设置
        $result =  Product::select('id','product_name')->get();
...
//    佣金的添加
    ...
    public function channel_record()
    {
        ...
        $product = Product::where(['status'=>1,'sale_status'=>0,'delete_id'=>0])->get();
...
//            $q->where('permission_id',3);  //渴求式加载
    ...
    {
        //获取所有的产品
        $product_list = Product::paginate(config('list_num.backend.product'));
