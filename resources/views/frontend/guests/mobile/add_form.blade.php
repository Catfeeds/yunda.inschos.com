...
<html>
	...
	<form class="form-horizontal" method="post"  id="form" action="{{ url('product/confirmform')}}">
		{{ csrf_field()}}
									...
									@if($v['module_key'] == 'ty_base')
										...
										@foreach($v['productAttributes'] as $vk => $vv)
												...
												@if($vv['type'] == 3)
														<li class="pickerfour">
															<span class="name">{{$vv['name']}}</span>
															<input id="{{$vv['ty_key']}}"  type="text" class="common-input" placeholder="{{$vv['name']}}" name="insurance_attributes[{{ $v['module_key'] }}][{{ $vv['ty_key'] }}]">
														</li>
													@endif
												@if($vv['type'] == 4)
													{{--todo 城市联动--}}
												...
												@if($vv['type'] == 5)
														<li class="pickerfive profession">
															...
															<input type="text" placeholder="请选择"/>
															<input class="inputhidden" hidden name="name="nsurance_attributes[{{ $v['module_key'] }}][{{ $vv['ty_key'] }}]"" type="text" value=""/>
															<i class="iconfont icon-gengduo"></i>
														</li>
												@endif
												@if($vv['type'] == 6)
														@foreach($vv['attributeValues'] as $ak => $av)
															<li class="pickerfour">
																<span class="name">{{$vv['name']}}</span>
																<input id="{{$vv['ty_key']}}" class="common-input btn btn-default" name="insurance_attributes[{{ $v['module_key'] }}][{{ $vv['ty_key'] }}]" value="{{$av['ty_value']}}" />{{$av['value']}}
															</li>
														@endforeach
												@endif
											...
											<span>{{ $v['name'] }}</span>
												...
												@foreach($v['productAttributes'] as $vk => $vv)
														...
														@if($vv['type'] == 3)
																<li class="approve">
																	<span class="name">{{$vv['name']}}</span>
																	<input id="{{$vv['ty_key']}}" type="text" class="w320"  class="common-input" placeholder="{{$vv['name']}}" name="insurance_attributes[{{ $v['module_key'] }}][{{ $vv['ty_key'] }}]">
																</li>
															@endif
															@if($vv['type'] == 4)
																{{--todo 城市联动--}}
																<li class="pickerfive area">
																	<span class="name">投保人所在地</span>
																	<input type="text" placeholder="请选择"/>
																	<input class="inputhidden" hidden name="{{$vv['name']}}" name="insurance_attributes[{{ $v['module_key'] }}][{{ $vv['ty_key'] }}]" type="text" value=""/>
																	<i class="iconfont icon-gengduo"></i>
																</li>
															@endif
															@if($vv['type'] == 5)
									...
									@elseif ($v['module_key'] == 'ty_beibaoren')
										...
										@foreach($v['productAttributes'] as $vk => $vv)
											...
											@if($vv['type'] == 3)
												<li class="approve">
													<span class="name">{{$vv['name']}}</span>
													<input id="{{$vv['ty_key']}}" type="text" class="w320"  class="common-input" placeholder="{{$vv['name']}}" name="insurance_attributes[{{ $v['module_key'] }}][{{ $beibaoren_key }}][{{ $vv['ty_key'] }}]" >
												</li>
											@endif
												@if($vv['type'] == 4)
													{{--todo 城市联动--}}
											...
											@if($vv['type'] == 6)
												<li class="approve">
													<span class="name">{{$vv['name']}}</span>
													@foreach($vv['attributeValues'] as $ak => $av)
														<input id="{{$vv['ty_key']}}"  class="btn btn-default" name="insurance_attributes[{{ $v['module_key'] }}][{{ $beibaoren_key }}][{{ $vv['ty_key'] }}]"  value="{{$av['ty_value']}}" />{{$av['value']}}
													@endforeach
												</li>
											@endif
											@if(in_array($vv['type'], [0, 9]))
