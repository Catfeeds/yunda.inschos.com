...
class FinanceController extends BaseController
{
    ...
    public function index()
    {
        ...
        /**上一个月净收益*/
        $start_time = date('Y', time()) . (date('m', time()) - 1) . '-01 00:00:00';
        ...
        $second_month_finance['month'] = date('Y', time()) . '年' . (date('m', time()) - 1) . '月';

        /**前面第二个月净收益*/
        $start_time = date('Y', time()) . (date('m', time()) - 2) . '-01 00:00:00';
        //end_time同上
        $third_month_finance = $this->financeRepository->getFinance($start_time, $end_time);
        $third_month_finance = $third_month_finance['amount'];
        $third_month_finance['month'] = date('Y', time()) . '年' . (date('m', time()) - 2) . '月';
